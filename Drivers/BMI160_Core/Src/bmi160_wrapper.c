...
static void set_bmi160_Ares(void);
static void set_bmi160_Gres(void);
static void get_bmi160_Ares(void);
static void get_bmi160_Gres(void);

static struct bmi160_dev sensor;
static struct bmi160_sensor_data accel;
static struct bmi160_sensor_data gyro;
static struct bmi160_foc_conf foc_conf;
static struct bmi160_offsets offsets;
...
int8_t BMI160_init(void)
{
    ...
    sensor.accel_cfg.odr = BMI160_ACCEL_ODR_200HZ; //BMI160_ACCEL_ODR_200HZ
    ...
    sensor.gyro_cfg.odr = BMI160_GYRO_ODR_200HZ; //BMI160_GYRO_ODR_200HZ
...
int8_t bmi160ReadAccelGyro(BMI160_t *DataStruct, uint8_t ZeroVelocityUpdate)
{
    ...
    if (ZeroVelocityUpdate)
    {
        DataStruct->Gx_bias = DataStruct->Gx_bias * 0.99f + gx * 0.01f;
        DataStruct->Gy_bias = DataStruct->Gy_bias * 0.99f + gy * 0.01f;
        DataStruct->Gz_bias = DataStruct->Gz_bias * 0.99f + gz * 0.01f;
    }
    DataStruct->BMI160_Gx_f32 = gx - DataStruct->Gx_bias;
    DataStruct->BMI160_Gy_f32 = gy - DataStruct->Gy_bias;
    DataStruct->BMI160_Gz_f32 = gz - DataStruct->Gz_bias;
...
{
    int8_t rslt;
    double gyro_x_bias = 0.0;
    double gyro_y_bias = 0.0;
    ...
    for (int i = 0; i < GYRO_CALIB_SAMPLES; ++i)
    {
        ...
        if (rslt != BMI160_OK)
            return rslt;
        gyro_x_bias += (double)gyro.x / bmi160_gRes;
        gyro_y_bias += (double)gyro.y / bmi160_gRes;
        gyro_z_bias += (double)gyro.z / bmi160_gRes;
        sensor.delay_ms(5);
    }
    DataStruct->Gx_bias = gyro_x_bias / GYRO_CALIB_SAMPLES;
    DataStruct->Gy_bias = gyro_y_bias / GYRO_CALIB_SAMPLES;
    DataStruct->Gz_bias = gyro_z_bias / GYRO_CALIB_SAMPLES;
    return rslt;
}
